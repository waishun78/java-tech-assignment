...
import cz.cvut.fit.tjv.social_network.dao.PostInMemRepository;
...
public class PostService extends AbstractCrudService<Post, Long> {
    ...
    public Collection<Post> findLikedMoreThan(int likes) {
        return ((PostInMemRepository) repository).findByLikedGreaterThan(likes);
    }
    ...
    public Collection<Post> readAllByAuthor(String authorId) {
        return ((PostInMemRepository) repository).findAllByAuthorUsername(authorId); //typecast from AbstractFileRepository to PostFileRepository
    }
}
